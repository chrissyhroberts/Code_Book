...
author: "Chrissy h Roberts & Yonggang Li"
---
...
The general methods used in this set of examples are very well described in [this paper on Segmented regression analysis of interrupted time series studies in medication use research, by Anita K. Wagner and colleagues](https://onlinelibrary.wiley.com/doi/full/10.1046/j.1365-2710.2002.00430.x?sid=nlm%3Apubmed) and I recommend that you start by reading that paper at least twice.
...
This tells us that the modelled line passes the y axis at 295.8 units and that prior to the intervention, the average value of quantity.x was falling by 2.01 units per week. At the intervention, there was a step change of -34.4 units and subsequent to the intervention there was a trend in which the value of quantity.x fell by *an additional* 0.94 units per week. The last distinction is important because that means that after the intervention, the weekly decrease in quantity.x was 2.01 + 0.94 = 2.95
...
**quantity.x\[i\] = 295.79584+ (Time\[i\]\*-2.01474) + (Intervention\[i\]\*-34.38002) + (Post.intervention.time\[i\]\*-0.93919)**

**quantity.x\[ii\] = 295.79584 + (Time\[ii\]\*-2.01474) + (Intervention\[ii\]\*-34.38002) + (Post.intervention.time\[ii\]\*-0.93919)**
...
This is annual (weekly) data, so the longest lag `max_lag` we could imagine would be a full year 52 week cycle. If it were monthly data, `max_lag` would likely be 12.
...
#### Plot **Partial Autocorrelation Function** PACF residuals
...
### A gls model with a corARMA correction
...
### A More Exhaustive search for the best autocorrelation values
...
## Look how MA/AR corrections affect results
