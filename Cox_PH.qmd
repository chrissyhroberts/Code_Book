...
Instead switch to an accelerated failure time model including a sex × period interaction to estimate difference-in-differences effects.
...
Lead to inaccurate estimates of absolute survival, but often still preserves relative differences (like hazard ratios or time ratios) if the truncation is balanced across groups
...
```{r}
exp(coef(aft_model)["male:post_periodTRUE"])
